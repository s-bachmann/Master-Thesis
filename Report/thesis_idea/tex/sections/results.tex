...
In the main wavefront observed, it's very noticeable that the particle velocity changes polarity at different time- and space-steps, which is consistent with an oscialltory wave passing: peaks of the wave are followed by throughs, which leads to velocities of opposite polarities. This is also noticeable in the cases that have multiple wavefronts, although it's less pronounced. \par
...
\begin{figure}[H]
    ...
    \caption{CZM Supershear strain and strain rate (horizontal component) averaged over 10.4 m gaugue length to represent DAS data: strain rate in top column, strain in bottom column. Crack depth on left side, snow bottom on right side. Colorbar shows magnitude of strain or strain rate, colorbars unscaled.}
...
In the vertical velocity plots in the top row of figure \ref{fig:mpm_vel}, the secondary wavefronts are very strong and can be seen clearly at both depths. They originate in a v-shape from the crack initiation point at x=75m. These wavefronts are very strong in amplitude (saturated colors), and show the propagation of a periodic osciallatory wave. The wvaefronts in both the positive and negative direction of propagation widen as time increases and their amplitude decreases, which is indicative of gemetric spreading and dispersion. When looking at the later times in the top panels of figure \ref{fig:mpm_vel}, the blue and red velocities seem to curve towards the origin of the plot (towards x=0). This is another indicator for dispersion of higher frequencies. \par
