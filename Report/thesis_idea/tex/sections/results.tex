...
The results shown here are from the MPM-guided simulation. The sources are spaced 0.01m apart from 75m to 125m. Unlike the results in section \ref{sec:results-1}, which are for a simple, 2 layer model with prescribed rupture velocities (sub-rayleigh and supershear case), the results here are for a 5-layer model with a rupture velocity coming from the MPM simulations. 

\subsection{MPM Strain and Strain rate}
\label{sec:mpm_strain}

\textcolor{magenta}{RERUN PLOTS BECAUSE TITLES AND ORDER IS WRONG}
