...
\begin{tikzpicture}[node distance=1.5cm and 2.5cm, every node/.style={text centered}]
    % ==========================================================
% SECTION 1: Pre-processing (Python / MPM)
% ==========================================================
\node (start) [startstop] {MPM Output:\\Stress, Strain, J-Value};

% Flow moves right
\node (pro1) [process, right of=start, xshift=2cm] {Filter plastic\\deformation/waves\\(by J-Value)};
\node (pro3) [process, right of=pro1, xshift=2cm] {Calculate stresses\\in individual\\directions};

% Data inputs branch down
\node (pro2) [io, below of=pro1, yshift=-0.5cm] {Material properties\\(Stretch factor)};
\node (pro4) [io, below of=pro3, yshift=-0.5cm] {Particle volume};

% Final output of this section
\node (pro5) [startstop, right of=pro3, xshift=2cm] {Moment tensor\\components\\(every direction)};



% ==========================================================
% SECTION 2: SALVUS Setup
% ==========================================================
% Flow moves right from pro5, starting the new group
\node (pro7) [process, right of=pro5, xshift=2.5cm] {FILTERING};
\node (pro8) [process, right of=pro7, xshift=1.5cm] {Pad coordinates\\away from\\ABCs};
\node (stop) [startstop, right of=pro8, xshift=1.5cm] {Setup wavelet (STF),\\pass to SALVUS};
...
% Input data flows (angled)
\draw [arrow] (pro2) -- (pro1);
\draw [arrow] (pro4) -- (pro3);

% Cross-group flow (thick arrow to imply data transfer)
\draw [arrow, line width=1.5pt, dash pattern=on 5pt off 2pt] (pro5) -- node[anchor=south, yshift=2pt, text=black!70] {Load Data} (pro7);
...
\end{tikzpicture}
\caption{Workflow for conversion of MPM data and setup as STF in SALVUS.}
