...
\begin{tikzpicture}[node distance=1.5cm]
    ...
    \node(pro1)[io, below of = in1] {Speed definition: 1.6vs (supershear), 0.6vs (sub rayleigh, according to \cite{trottetTransitionSubRayleighAnticrack2022})};
    \node(pro2)[process, below of =pro1] {Calculation of time delay between sources: $\ \Delta t = \frac{\Delta x}{v_{rupture}}$};
    \node(pro3)[io, below of= pro2] {Input of base values for all directions of moment tensor (e.g. mxx = 1)};
    \node(pro4)[io, below of=pro3] {Input of weight arrays: Calculated from MPM data or 1 in all directions};
    \node(pro5)[process, below of=pro4]{Tapering of first and last sources to avoid injection noise};
    \node(pro6)[process, below of =pro5]{Zero-padding of wavelet arrays before and after their firing time};
    \node(pro7)[process, below of=pro6]{Calculation of source wavelet shape with original shape, weight, padding and taper};
