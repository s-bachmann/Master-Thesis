...
\label{sec:DAS_noise}
To assess the field detectability of the signatures measured in this thesis, a comparison with the noise spectral density of the Silixa iDAS instrument \parencite{parkerDistributedAcousticSensing2014, idasdatasheet} was done. This instrument was chosen because it was used for avalanche detection in the Vallée de la Sionne test site in the Swiss Alps by \cite{paitzPhenomenologyAvalancheRecordings2023, edmeFiberopticDetectionSnow2023}. For a gauge length of 10m, similar to the one used by \cite{turquetAutomatedSnowAvalanche2024} and this thesis, the amplitude spectral density, which describes how much noise is present per frequency bandwidth \parencite{hartogIntroductionDistributedOptical2017}, is $\ 2 p\epsilon per \sqrt{Hz}$ \parencite{idasdatasheet}. This noise floor is used to calculate the signal to noise ratio for the results in sections \ref{sec:results-1}, \ref{sec:mpm_results} as described in section \ref{sec:das_theory}. The input values for equations \ref{eq:rms_noise} and \ref{eq:snr} can be seen in table \ref{tab:snr_input}. 

 \begin{table}[H]
\begin{tabular}{|l|l|}
\hline
Parameter                                                                            & Input Value                                                                                \\ \hline
Amplitude Spectral Density                                                           & \begin{tabular}[c]{@{}l@{}}$\\ \ 2\\  p\\ \epsilon\\  per \\ \sqrt\\ {Hz}\\ $\end{tabular} \\ \hline
$\ \Delta f$ Simple Case                                                             & 50 Hz                                                                                      \\ \hline
$\ \Delta f$ MPM                                                                     & 30 Hz                                                                                      \\ \hline
$\ \epsilon_{peak}$ Simple, Sub-Rayleigh                                             & $/ 3* 10^{-6}$                                                                             \\ \hline
\begin{tabular}[c]{@{}l@{}}$\ \\ \epsilon_{peak}\\ $ Simple, Supershear\end{tabular} & $/ 3* 10^{-5}$                                                                             \\ \hline
\begin{tabular}[c]{@{}l@{}}$\ \\ \epsilon\\ _\\ {peak}$ MPM\end{tabular}             & $/ 2* 10^{-5}$                                                                             \\ \hline
...
\begin{tabular}{|l|l|l|}
\hline
Case                & $\ RMS_{Noise}$, from equation \textbackslash{}ref\{eq:rms\_noise\} & SNR, from equation \textbackslash{}ref\{eq:snr\} \\ \hline
Simple Sub-Rayleigh & 0.0141n$\ \epsilon$                                                 & 212'132                                          \\ \hline
Simple Supershear   & 0.0141n$\ \epsilon$                                                 & 2'121'320                                        \\ \hline
MPM                 & 0.0110n$\ \epsilon$                                                 & 1'825'742                                        \\ \hline
