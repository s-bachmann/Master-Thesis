...
The quasi-static wavefield is contained within a v-shapedd wavefront originating from the crack onset at x=75m. It shows high amplitude lines of positive and negative strain which are alternating. In the positive
...
(table \ref{tab:mpm_params}), which are also close to the A0 and S0 velocities in the slab, the combined energy band appears broader than either line alone. This is consistent with the same precursor observed in field measurements by \textcite{herwijnenSeismicSensorArray2011}, where a fast P/S0-wave arrival preceded the main crack signal. \par
...
Additional to environmental noise, there is also instrument noise, which is caused by changes in the laser pulse or interference of the signal as it travels through the cable for example \parencite{lindseyFiberOpticSeismology2021}. This poses the question if the signatures discussed can even be discovered against the instrument noise, even when there is no environmental disturbance. An assesment for the instrumental noise floor of the Silixa iDAS system used by \cite{paitzPhenomenologyAvalancheRecordings2023} is done in section \ref{sec:DAS_noise}. \par
...
% ===============================
    ...
    \node[process, text width=5.2cm, minimum height=1.8cm, align=left] (leftbox) at (-3.6, -1.8) {
        \begin{itemize}
          ...
          \item Symmetric v-shapedd Arrest Signature
          \item A0 Coda 
        \end{itemize}
    };
