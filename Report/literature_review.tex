...
\begin{description}
  \item[$t_o$]: Subfault onset time [s], e.g. $t_o=(x-x_{\rm start})/v_{\rm rupture}$.
  \item[$x,\,x_{\rm start}$]: Horizontal coordinate and rupture starting position [m].
  \item[$v_{\rm rupture}$]: Prescribed rupture propagation speed [m s$^{-1}$].
  \item[$t_{rise}$]: Rise time of a subfault/source [s].
  \item[$\Delta x$]: Subfault width or discretisation length in x [m].
  \item[$x_{\rm slip}$]: Prescribed slip on a subfault [m].
  \item[$M_0$]: Scalar seismic moment per subfault [N m].
  \item[$m_{ij}$]: Moment tensor components [N m]. 
  \item[$r_{\rm mode}$]: Mode mix parameter [dimensionless], $r_{\rm mode}=0$ (Mode I) to $1$ (Mode II).
  \item[$l_{cz}$]: Cohesive process (fracture) zone length [m]. Related to $l_{ch}$ via $l_{cz}=\tfrac{\pi}{8}l_{ch}$.
  \item[$l_{ch}$]: Irwin characteristic length [m], $l_{ch}=\dfrac{E_{\rm eff}G_{II}}{\tau_s^2}$.
  \item[$\lambda_{\min}$]: Shortest wavelength to resolve [m], $\lambda_{\min}=v_{\min}/f_{95}$.
  \item[$v_{\min},v_{\max}$]: Minimum and maximum relevant wave speeds in the model (m s$^{-1}$).
  \item[$\Delta t$]: Time step [s] for explicit time integration.
  \item[$\xi$]: Normalized opening [dimensionless].
  \item[$\delta$]: Local opening (separation) displacement across the interface [m].
  \item[$\delta_c$]: Mixed-mode critical opening [m] used to nondimensionalize openings.
  \item[$\delta_{n,c},\,\delta_{s,c}$] Single-mode critical openings for Mode I (normal) and Mode II (shear) [m].
  \item[$T(\xi)$]: Cohesive traction as a function of normalised opening [Pa or N m$^{-2}$].
  \item[$T_{\mathrm{peak}}$]: Peak cohesive traction [Pa].
  \item[$\phi(\Delta)$] Mixed-mode cohesive potential [J m$^{-2}$] with $\Delta=(\Delta_n,\Delta_t)$.
  \item[$\Delta_n,\,\Delta_t$]: Normal and tangential separations [m].
  \item[$\delta_n,\,\delta_t$]: Normal and tangential length scales in the cohesive potential [m].
  \item[$q$] Coupling parameter in the mixed-mode cohesive potential [dimensionless].
  \item[$\phi_n,\,\phi_t$]: Work (energies) of separation for pure normal and tangential modes [J m$^{-2}$].
  \item[$\sigma_{\max},\,\tau_{\max}$]: Peak normal strength and peak shear strength [Pa]. 
  \item [$\sigma_n$, $\tau_s$] Normal and shear strength [Pa].
  \item[$G_I,\,G_{II}$]: Fracture energies for Mode I and Mode II [J m$^{-2}$].
  \item[$\sigma_n,\,\tau_s$]: Nominal normal and shear strengths [Pa]. 
  \item[$v_s,\,v_p$]: Pressure and Shear seismic wave speeds [m s$^{-1}$].
  \item[$\rho$]: Mass density [kg m$^{-3}$].
  \item[$\mu,\,\lambda$] Lamé parameters [Pa]
  \item[$E$]: Young's modulus [Pa]
  \item[$l_{crack},\,x_0$]: Local crack process length and initial crack front position [m] used in local coordinate definitions.
\end{description}
...
\section{Methods - Cohesive Zone Model}
...
$G_I, G_{II}$ & From \cite{mahajanModelingInterfacialCrack2008} (used to compute $\delta_{n,c},\delta_{s,c}$) \\
$v_s, v_p, \rho$ & From \cite{guillemotEffectSnowfallChanges2021} (converted to $\mu,\lambda,E$) \\
$r_{\rm mode}$ & User input (0 = Mode I, 1 = Mode II) \\
$l_{cz}$ & $l_{cz}=\tfrac{\pi}{8}l_{ch}$ with $l_{ch}=\dfrac{E_{\rm eff}G_{II}}{\tau_s^2}$ \\
\hline
\end{tabular}
\end{center}
