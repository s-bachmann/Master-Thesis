...
\textbf{NOTE: } parameters $\ \tau_s$, cohesive zone length and critical slip were scaled by the same factor to avoid numerical resolution issues (eg to ensure that the process zone is resolved in the mesh) the scaling was seen as the best option as to not increase computation time and memory requirements by densifying the emsh 
