...
{\centering \Large \textbf{From Particles to Waves: Towards coupling Dynamic Snow Modelling and Spectral Elements to detect Avalanche Breakoff in DAS Data.}\par}

\section{Nomenclature}
\subsection{List of Abbreviations}
\begin{description}
  \item \textbf{DAS} : Distributed Acoustic Sensing
  \item \textbf{MPM} : Material Point Method
  \item \textbf{DEM} : Discrete Element Method
  \item \textbf{FE, FEM} : Finite Element Method
...
\begin{description}
  ...
  \item[$\boldsymbol{\theta}$]: Reference coordinate system in respect to mesh elements used in SALVUS. 
  \item[$\boldsymbol{w}$]:: Test function used to obtain the weak form of FE equations [dimensionless]. 
  \item[$\boldsymbol{G}$]: Model domain [dimensionless]. 
  \item[$\boldsymbol{V}$]: Galerkin space [dimensionless].
  \item[$\boldsymbol{u}$]: Scalar displacement potential [dimensionless]. 
  \item[$\boldsymbol{\phi}$]: Set of functions which approximate solution of w or u [dimensionless].
  \item[$\boldsymbol{F}$]: Forcing term of elastic wave equation.
  \item[$\boldsymbol{M}$]: Mass term of elastic wave equation.
  \item[$\boldsymbol{S}$]: Stiffness term of elastic wave equation.
  \item[$\boldsymbol{B}$]: Boundary term of elastic wave equation.  
  \item[$\boldsymbol{\partial}$]: Partial derivative in respect to one variable. 
\end{description}
\newpage
...
The same is true for finite element modelling (FE or FEM), which is done using SALVUS in this Thesis. FEM has been used to model snow mechanics since the 1970s, a review of the different approaches has been published by \cite{podolskiyReviewFiniteelementModelling2013}. The finite element modelling approach used in this thesis is described more in detail in \ref{sec:salvus method}. FE can also not account for anticrack propagation, similarly to DEM, mesh elements would need to be removed to account for anticrack propagation. \par
...
\end{equation} \parencite{afanasievModularFlexibleSpectralelement2019,kAkiRichards19801981}. Stress can be related to displacement using Hoek's law: $\ \sigma = \frac{C}{\nabla u}$, where C is a 4th order tensor which characterizes the stiffness of the medium \parencite{afanasievModularFlexibleSpectralelement2019}. Combining \ref{eq:waveeq simple} with Hoek's law, this leads to \begin{equation}
\rho\partial_t^2u = \nabla \cdot \frac{C}{\nabla u} + f,
...
\end{equation} \parencite{afanasievModularFlexibleSpectralelement2019}. This means that fields such as displacement and velocity are calculated in this way. From these fields, the data from the model in section \ref{sec:czm methods} is obtained and plotted. 

\subsection{Model setups}
\label{sec:simple model}
The same two-layer medium as in section \ref{sec:czm methods} was set up. In the simple case, a series of SALVUS moment tensor sources are set up with a certain distance. The time delay with which the sources fire is dependend on the sub-rayleigh or supershear case which is implemented. The bedrock or soil is not considered in both models. This is because preliminary tests of a 3-layered medium have shown that it acts as a transmitter of waves. For computational efficiency, the bedrock layer was thus neglected and the bottom of the snow was made a regular boundary,in other words, reflecting, to mimick the presence of a layer below the snow. 
...
\section{Results}
\begin{itemize}
  \item For all cases show: \textbf{Space time plots:} Strain (x) rate plots, Strain (x), velocity, in all cases at crack depth and snow bottom. \textbf{FK plots with velocites drawn}, \textbf{wiggle plots or single traces}, possibly PSD and STF plots to support certain cases, but i'm not sure these are needed because they look similar in all cases? 
  \item Put animations via link, stills of all animation cases 
  \item Results of Complex Model (Cohesive zone) in FE, supersher vs sub-rayleigh, possibly weak layer vs no weak layer
  \item Results for simple FE model, all the same comparisons
  \item Results of MPM-based model, all the same comparisons
  \item MAYBE: show some tests of weighted / custom stf so process can be followed better
\end{itemize}

\section{Discussion}
\begin{itemize}
  \item Compare results of all models with wave physics (e.g what equations can explain which phenomena and interactions of the waves)
  \item Lamb waves physics
  \item Timoshenko beam effects MAYBE????
  \item FE vs MPM model discussion explained by different methodology, or find another way to explain differences
  \item I would like to lead from my input wave equation to the resulting waves observed in the results in a mathematrical way, eg show the input equation, all the interactions and then the resulting equations of for example lamb waves: 
  \item From the equation of motion in an elastic solid, which is \begin{equation}
  ...
  \end{equation} for S-waves, the wave speeds are derived from the Lamé-Parameters, as described in \ref{sec:czm methods}. \par
  ...
  \end{equation} and \begin{equation}
    \sigma_{xz} = \mu [2 \frac{\partial^2 \phi}{\partialx \partial z} + \frac{\partial^2 \psi}{\partial x^2} - \frac{\partial^2 \psi}{\partial z^2}], 
  ...
  \item Copmare a figure of lamb wave propagation of the different modes to the results obtained in GIFS to clearly show that they are presnet: 
  \item In space time plots: describe the different wavefronts which can be seen. In sub-rayleigh, slope of stongest main front is sources firing (same as propagation speed), wavefront which travels ahead of this is smaller. In supershear, only one distinct front and backwards propagation: WHICH TYPES OF WAVES ARE THEY AND WHY 
  \item Possibly link different observations to physical processes (eg leading wavefronts) that could be useful in detecting slab breakoff in real-world data
  \item Also show fk plots of all the case and discuss which speed dominates when (eg in sub-rayleigh it's the slab speed) \rightarrow possibly timoshenko effects here because of the 4th order term when frequency goes higher approximates medium speed ? I think i'll explain this primarily with the highest speed present: in the sub-rayleigh case, the p-wave speed of the slab is the highest speed in the system, in the supershear case, the highest speed present is the crack propagation speed. 
  \item I would like to discuss the implications of the different wave types: eg have a clear, physical explaiantion for what different types of waves mean and how this is shown in terms of the crack or the snowpack \rightarrow the lamb waves are caused by the impedance contrast at snow-air causing a stress free-boundary, which then guides the waves (link to derivation)
  \item Tie all of this to literature of fracutre mechanics and avalanche modelling, to also show where the phenomena come from and they could for example show up in not seismic data 
  \item Important caveat: different stages of crack propagation as described by \cite{bobillierNumericalInvestigationCrack2024} are irrelevant here, because crack propagates at constant speed \rightarrow this is also not very physical and an important consideration for comparison to data as well as interpretation
\end{itemize}

\section{Conlcusion}
\begin{itemize}
  \item Link between all different models and scenarios and big takeaways for DAS data
  \item Summary of observations, compare and contrast models
  \item What can be built on this? eg more accurate Salvus model with multiple layers and topography etc
  \item also add cracks not at steady state speed: crack propagation in transition mechanisms described in introduction 
  \item Slope effects will probably change results a lot
  \item It would be nice to have a "how to detect slab braekoff in DAS data" here, for example what types of waves to look for, what order they have etc
\end{itemize}

\section{Notes for Results \& Discussion}

\subsection{Slab Mechanics}
\cite{sironTheoreticalFrameworkDynamic2023} model their snow slab as a Timoshenko beam. Such beams are thick beams, sandwich composite beams, or beams where the wavelength is similar to the beam thickness \parencite{sironTheoreticalFrameworkDynamic2023}. With a central frequency of $\ f_0 = 20 Hz$, the wavelength of the source used in this thesis is $\ \nu = \frac{\lambda}{v}$, where $\ \nu$ is the frequency and v is the speed of the medium. This leads to a central frequency wavelength of 15m for the shear wave and 30m for the compressional wave, this means the snow slab layer can be seen as a Timoshenko beam. According to \cite{meiWaveReflectionTransmission2004}, Timoshenko beams act as waveguides. In contrast to Euler-Bernoulli models, which only consider lateral inertia and elastic forces caused by bending deflections, Timoshenko beams also consider rotary inertia and shear distortion \parencite{meiWaveReflectionTransmission2004}. This is relevant for snow slabs because they can be distorted under loading which can cause avalanches. The equation of motions for a beam element along the x-axis in a Timoshenko beam are: \begin{equation}
  GA\kappa [\frac{\partial \psi (x,t)}{\partial x} - \frac{\partial^2 y (x,t)}{\partial x^2}] + \rho A \frac{\partial^2 y(x,t)}{\partial t^2} = q(x,t)
  \label{eq: timoshenko motion 1}
\end{equation} and \begin{equation}
  EI \frac{\partial^2 \psi (x,t)}{\partial x^2} + GA\kappa [\frac{\partial y (x,t)}{\partial x} - \psi(x,t)] - \rho I \frac{\partial^2y(x,t)}{\partial t^2} = 0
  \label{eq: timoshenko motion 2}
\end{equation} \parencite{meiWaveReflectionTransmission2004}. In equations \ref{eq: timoshenko motion 1,eq: timoshenko motion 2} $\ \frac{\partial y (x,t)}{\partial x},  \frac{\partial y (x,t)}{\partial x} - \psi(x,t)$ are the slope of the center axis of the beam and the shear angle \parencite{meiWaveReflectionTransmission2004}. Both equations of motion are coupled through the slope and the transversal (across) deflextion of the beam, the shear force and bending moment are also related to these two \parencite{meiWaveReflectionTransmission2004}. The shear force V(x,t) is related according to \begin{equation}
  V(x,t) = GA\kappa [\frac{\partial y (x,t)}{\partial x} - \psi(x,t)]
  \label{eq:timoshenko shear}
\end{equation} and the bending moment M(x,t) according to \begin{equation}
  M(x,t) = -EI\frac{\partial\psi(x,t)}{\partial x}
  \label{eq:timoshenko bending}
\end{equation} \parencite{meiWaveReflectionTransmission2004}. \par

The propagation matrices of sources at points A and B can be described by the vectors of wave propagation in positive and negative direction of propagation $\ a^+, a^-, b^+, b^-$ \parencite{meiWaveReflectionTransmission2004}. Using the propagation matrix for distance x which is $\ f(x) = \begin{bmatrix}
{e^{-ik_1x} & 0 \\ 0 & e^{-ik_2x}\\} \end{bmatrix}$, the vectors can be related according to $\ a^- = f(x)b^-, b^+ = f(x)a^+$  \parencite{meiWaveReflectionTransmission2004}. A similar matrix can be obtained for the reflections at boundaries, assuming equilibrium of shear and bending at the boundary \parencite{meiWaveReflectionTransmission2004}. This can be considered as \begin{equation} -V = HA\kappa (\frac{\partial y}{\partial x} \psi) = \bar{K}_{TY} + \bar{K}_{TR\psi} \end{equation} and \begin{equation}
  M = -EI\frac{\partial y}{\partial x} = \bar{K}_{R\psi} + \bar{K}_{RTY}
\end{equation} \parencite{meiWaveReflectionTransmission2004}. This means that both incident propagating and transitonal wave modes are fully reflected, keeping their original wave types but with a 180 degree phase change \parencite{meiWaveReflectionTransmission2004}. This is important to consider when looking at results of reflections in the snow slab. At a free surface, which the snow-air interface is, the phase angle of the reflected wave is frequency dependent, and a propagation mode arises from the incident transition mode \parencite{meiWaveReflectionTransmission2004}. 



\subsection{Lamb Wave propagation Physics}
Lamb waves are elastic guided waves that occur in plates \parencite{suFundamentalsAnalysisLamb2009}. They consist of particle motion in the direction of the wave polarization as well as the direction perpendicular to it \parencite{suFundamentalsAnalysisLamb2009}. They are guided by the boundaries of the layer or media in which they propagate and have a symmetric and an asymmetric mode \parencite{suFundamentalsAnalysisLamb2009}. The symmetric mode dominates the in-plate motion, while the antisymmetric mode dominates the vertical (out of plane motion ) \parencite{suFundamentalsAnalysisLamb2009}. Lamb waves are dispersive. Lamb waves in a plate with thickness d and half thickness h can be described as \begin{equation}
  \frac{tan(qh)}{tan(ph)} = \frac{4k^2qp\mu}{(\lambda k^2 + \lambda p^2 + 2\mu p^2)(k^2-q^2)}
  \label{eq:lamb wave}
\end{equation} \parencite{suFundamentalsAnalysisLamb2009}. This can be split up into \begin{equation}
  \frac{tan(qh)}{tan(ph)} = -\frac{4k^2qp}{(k^2-q^2)^2}
  \label{eq:lamb symmetric}
\end{equation} for symmetric and \begin{equation}
  \frac{tan(qh)}{tan(ph) = -\frac{(k^2-q^2)^2}{4k^2qp}}
  \label{eq:lamb antisymmetric}
\end{equation} for antisymmetric modes \parencite{suFundamentalsAnalysisLamb2009}. In isotropic materials these equations are \begin{equation}
  \frac{tan(qh)}{q} = \frac{4k^2ptan(ph)}{(k^2-q^2)^2 = 0}
  \label{eq:isotropic symmetric lamb}
\end{equation} in the symmetric case and \begin{equation}
  qtan(qh) + \frac{(k^2-q^2)^2 tan(ph)}{4k^2p} = 0
  \label{eq:isotropic asymetric lamb}
\end{equation} for the antisymetric waves \parencite{suFundamentalsAnalysisLamb2009}. These are also the dispersion equations of the two Lamb wave modes , the symmetrical mode is more dispersive than the antisymmetric mode because of the division by q in equation \ref{eq:isotropic symmetric lamb} \parencite{suFundamentalsAnalysisLamb2009}. 
