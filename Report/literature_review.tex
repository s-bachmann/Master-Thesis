...
The propagation matrices of sources at points A and B can be described by the vectors of wave propagation in positive and negative direction of propagation $\ a^+, a^-, b^+, b^-$ \parencite{meiWaveReflectionTransmission2004}. Using the propagation matrix for distance x which is $\ f(x) = \matrix{e^{-ik_1x}\  0 \cr 0 \ e^{-ik_2x}\cr}$, the vectors can be related according to $\ a^- = f(x)b^-, b^+ = f(x)a^+$  \parencite{meiWaveReflectionTransmission2004}. 
