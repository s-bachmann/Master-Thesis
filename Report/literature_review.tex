...
As mentioned, the model domain needs to be mapped to real-world coordinates. In this thesis, the domain consists of a snow layer and an air layer, which means there is a sharp discountinuity in model parameters. As \cite{afanasievModularFlexibleSpectralelement2019} state, the model parameters and strain field are not differentiable. This is why a smooth polynomial basis of the governing equations over the whole domain is not appropriate, the basis B needs to consist of piecewise functions G defined on the individual elements \parencite{afanasievModularFlexibleSpectralelement2019}. 
